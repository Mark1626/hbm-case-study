...
static inline hipError_t checkCudaErrors(hipError_t result) {
...
__global__ void bitonicSortKernel(float *in, float *out, uint arrayLength,
                                  ...
                                  uint batchX, uint batchY, uint batchSize2D,
                                  uint wdim, uint noOfBatches) {
...
void bitonic_sliding_median(float *median, float *madfm, float *arr,
  ...
  checkCudaErrors(hipMalloc(&d_arr, arraySize * sizeof(float)));
  checkCudaErrors(hipMalloc(&d_median, noOfWindows * sizeof(float)));
  checkCudaErrors(hipMalloc(&d_madfm, noOfWindows * sizeof(float)));
  checkCudaErrors(hipMemcpyAsync(d_arr, arr, arraySize * sizeof(float),
                                 hipMemcpyHostToDevice));

  checkCudaErrors(hipMalloc(&d_windows, bufferSize * sizeof(float)));
  ...
  for (uint batch = 0; batch < noOfBatches; batch++) {
...
#endif
    hipLaunchKernelGGL(load_2d_kernel, batchDim2D, threadCount2D, 0, 0,
                       d_windows, d_arr, arrayDim, batchX, batchY, batchDim,
                       noOfBlocks2D);

#ifdef DEBUG
    {
      int row = 1;
      checkCudaErrors(hipMemcpy(h_windows, d_windows,
    ...
    hipLaunchKernelGGL(bitonicSortKernel, batchSize, threadCount, 0, 0,
                       d_windows, d_median, TOTAL_WINDOW_SIZE, dir, batch,
                       batchSize, batchX, batchY, batchDim2D.x, wdim,
                       noOfBlocks);

    hipLaunchKernelGGL(bitonicSortKernel, batchSize, threadCount, 0, 0,
                       d_windows, d_madfm, TOTAL_WINDOW_SIZE, dir, batch,
                       batchSize, batchX, batchY, batchDim2D.x, wdim,
                       noOfBlocks);
  }

  checkCudaErrors(hipMemcpyAsync(median, d_median, sizeof(float) * noOfWindows,
                                 hipMemcpyDeviceToHost));

  checkCudaErrors(hipMemcpyAsync(madfm, d_madfm, sizeof(float) * noOfWindows,
                                 hipMemcpyDeviceToHost));
  ...
  {
    int row = 1;
    checkCudaErrors(hipMemcpy(h_windows, d_windows, bufferSize * sizeof(float),
                              hipMemcpyDeviceToHost));
    printf("\nOutput After Sort:\n");
    int idx = 0;
    for (uint window = 0; window < batchSize; window++) {
  ...
  free(h_windows);
  checkCudaErrors(hipFree(d_median));
  checkCudaErrors(hipFree(d_madfm));
  checkCudaErrors(hipFree(d_windows));
